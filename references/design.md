Warning: truncated output (original token count: 20467)
...
Marp is an optional third path, alongside WeasyPrint HTML and python-pptx. Use it only when the user explicitly asks for Marp, "markdown slides", or a deck that lives in a `.md` file. The repo does not bundle…467 tokens truncated…g. cover) | `<!-- _class: cover -->` at the top of the section |
